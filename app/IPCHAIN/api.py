...
from app import ipchain_wallet
...
ipchain_ns = Namespace("ipchain", description="IPCHAIN冲提币接口")
...
class CreateAccount(Resource):
    ...
    def post(self):
        ...
        # digiccy account
        bind_secret, bind_account = ipchain_wallet.create_account()
...
class Withdraw(Resource):
    ...
    def post(self):
        ...
        if ipchain_wallet.is_syncing():
            return create_response(CodeMsg.CM(1020, 'IPCHAIN节点未同步,请勿执行提现操作'))

        # 提币地址校验
        if not ipchain_wallet.is_valid_address(digiccy_address):
