...
eos_ns = Namespace("eos", description="eos冲提币接口")
...
class Withdraw(Resource):
    ...
    def post(self):
        ...
        if not eos_wallet.init_eos():
            return create_response(CodeMsg.CM(1020, 'EOS节点未同步,请勿执行提现操作'))

        # 提币地址校验
        if not eos_wallet.is_valid_address(digiccy_address):
