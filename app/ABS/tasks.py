...
from app import db, abs_wallet, consul_clinet, stellar_service
    ...
    with db.app.app_context():
        ...
        while True:
            ...
            for user in users:
                ...
                # balance为0的账户跳过
                try:
                    balance = abs_wallet.get_balance(address)
                    if balance <= 0:
                        continue
                ...
                records = abs_wallet.eth_transaction_record(address, last_time, start_block=last_block)
                if records is None:
                    continue
...
def task_chain_out_withdraw():
    ...
    with db.app.app_context():
        ...
        # ABS base账户address
        abs_base_address = consul_clinet.get_digiccy_base_account(COIN_NAME, is_seed=False)
        # abs_base_seed = consul_clinet.get_digiccy_base_account(COIN_NAME)
        #
        ...
        for order in orders:
            ...
            # 本地节点是否链接,是否同步
            if not abs_wallet.is_connected():
                logging.error('{}节点离线!'.format(COIN_NAME))
                return
            if abs_wallet.is_syncing():
            ...
            # 链外账户余额
            base_account_balance = abs_wallet.get_balance(abs_base_address)  # decimal.Decimal
            minerfee = abs_wallet.get_minerFee(abs_base_address)
            if minerfee < 0.0001:
            ...
            # 付款账户信息
            # 当前区块
            block_num = abs_wallet.get_block_num()
            nonce_num = abs_wallet.get_nonce(abs_base_address)

            # 改变订单状态
            order.order_from = abs_base_address
            order.status = 0
            ...
            order_detail = OrderDetail(
                ...
                query_json=json.dumps(
                    dict(addrfrom=abs_base_address, addrto=order_to, block_num=block_num, nonce_num=nonce_num)),
                response_json='',
                status=-1
            )
            try:
                ...
                return
            is_success, msg = abs_wallet.payment(abs_base_address, order_to, amount)
...
def task_chain_out_confirm():
    ...
    with db.app.app_context():
        ...
        for order in orders:
            ...
            # 查询交易是否被确认
            is_confirm, is_success, msg, fee = abs_wallet.hash_get_detail(order_hash)
...
def task_chain_out_collect():
    ...
    with db.app.app_context():
        # 归集地址:项目方提供地址boss账户
        abs_boss_address = consul_clinet.get_digiccy_boss_address(COIN_NAME)
        ...
        while True:
            ...
            for user in users:
                ...
                # 用户绑定地址余额,小于最小归集数量跳过
                balance = round(abs_wallet.get_balance(user_digiccy_address), 7)
                ...
                miner_fee = abs_wallet.get_minerFee(user_digiccy_address)
                if miner_fee < miner_minfee:
                    is_success, tx_hash = abs_wallet.minerfee_transaction(minerfee_address, user_digiccy_address,
                                                                          miner_minfee)
                    if not is_success:
                ...
                # 创建裸交易发出
                is_success, msg = abs_wallet.raw_transaction(user_digiccy_secret, abs_boss_address,
