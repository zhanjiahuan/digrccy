...
abs_ns = Namespace("abs", description="ABS冲提币接口")
...
class CreateAccount(Resource):
    ...
    def post(self):
        ...
        # digiccy account
        bind_secret, bind_account = abs_wallet.create_account()
...
class Withdraw(Resource):
    ...
    def post(self):
        ...
        if abs_wallet.is_syncing():
            return create_response(CodeMsg.CM(1020, 'abs节点未同步,请勿执行提现操作'))

        # 提币地址校验
        if not abs_wallet.is_valid_address(digiccy_address):
