...
class VPCWallet():
    """VPC钱包节点"""
    VPC_CONTRACT_ADDRESS = "0x28C69cD429F785954Ae4B0263Db4d9871C033cA7"
    VPC_CONTRACT_ABI = "https://api.etherscan.io/api?module=contract&action=getabi&address={}"
    VPC_API_KEY = '6RJPUEST3V8UR737RS89YZQ9HZIYU73PA7'
    VPC_BLOCK_API = 'https://api.etherscan.io/api?module=proxy&action=eth_blockNumber'
    VPC_TX_API = "http://api.etherscan.io/api?module=account&action=tokentx&address={address}&startblock={start_block}&endblock={end_block}&sort={sort}&apikey={api_key}"
    ...
    def get_contract_abi(self):
        vpc_contract_abi = redisService.get("vpc_contract_abi")
        if not vpc_contract_abi:
            url = self.VPC_CONTRACT_ABI.format(self.VPC_CONTRACT_ADDRESS)
            try:
                ret = requests.get(url).json()
                vpc_contract_abi = ret.get("result")
            except Exception as e:
                return
            redisService.set("vpc_contract_abi",vpc_contract_abi)
        return vpc_contract_abi

    def init_consul(self,app, consul_client):
        self.passphrase = app.config.get('GETH_VPC_PASSPHRASE')
        self.consul_client = consul_client
        self.wallet_url = "http://47.102.40.120:8221"
        self.w3 = Web3(Web3.HTTPProvider(self.wallet_url))
        if not self.w3.isConnected():
            logging.error('Connect VPC wallet node fial')

        contract_abi = self.get_contract_abi()
        if contract_abi is None:
            logging.error('get VPC abi fail')
        try:
            self.token_contract = self.w3.eth.contract(address=self.VPC_CONTRACT_ADDRESS, abi=contract_abi)
            self.decimals = self.token_contract.functions.decimals().call()
        except Exception as e:
            logging.error('get VPC token_contract fail {}'.format(e))
    ...
    def is_connected(self):
        ...
        if self.w3 is None or not self.w3.isConnected():
            ...
            try:
                ...
                print('VPC contract connected failed:', str(e))
                return False
    ...
    def get_balance(self,address):
        ...
        try:
            ...
            logging.error('VPC node get balance error:{}'.format(str(e)))
            raise Exception('VPC node get balance error')
    ...
    def payment(self, addrfrom, addrto, amount):
        ...
        try:
            ...
            logging.error('VPC payment error:{}'.format(str(payload)))
            return False, str(e)
    ...
    def get_db_base_account(self):
        """数据库获取VPC base账户"""
        try:
            vpc_base_address = Config.digiccy_base_account("VPC")
        except Exception as e:
            logging.error('DB get digiccyVPCBaseAccount error:{}'.format(str(e)))
            raise Exception('DB get digiccyVPCBaseAccount error')
        return vpc_base_address

    def get_node_base_account(self):
        """本地节点获取VPC base账户"""
        try:
            vpc_base_address = self.w3.personal.listAccounts[0]
        except Exception as e:
    ...
    def get_mian_block_num(self):
        """获取公链上的最新区块数"""
        try:
            ret = session_pool.get(self.VPC_BLOCK_API, timeout=30).json()
    ...
    def eth_transaction_record(cls, address, last_timestamp, start_block=None, end_block=99999999, sort='asc', ):
        ...
        url = cls.VPC_TX_API.format(address=address,  # eth账户地址
                                    start_block=start_block,  # 开始区块
                                    end_block=end_block,  # 结束区块
                                    sort=sort,  # 排序 desc降序 asc 升序
                                    api_key=cls.VPC_API_KEY)  # eth api KEY
        try:
            ret = session_pool.get(url, timeout=10).json()
        except Exception as e:
            logging.error('Request VPC_TX_API error:{}'.format(str(e)))
            return
        ...
        if status == '1' and message == 'OK':
            for record in records:
                ...
                if contract_address != cls.VPC_CONTRACT_ADDRESS.lower():
                    continue
            ...
            if records is None:
                logging.error('Request VPC_TX_API error:{}'.format(str(ret)))
                return
