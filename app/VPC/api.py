...
vpc_ns = Namespace("vpc", description="VPC冲提币接口")
...
class CreateAccount(Resource):
    ...
    def post(self):
        ...
        # digiccy account
        bind_secret, bind_account = vpc_wallet.create_account()
...
class Withdraw(Resource):
    ...
    def post(self):
        ...
        if vpc_wallet.is_syncing():
            return create_response(CodeMsg.CM(1020, 'vpc节点未同步,请勿执行提现操作'))

        # 提币地址校验
        if not vpc_wallet.is_valid_address(digiccy_address):
        ...
        if ret.code == CodeMsg.SUCCESS.code:
            ...
            return create_response(CodeMsg.CM(200, '下单成功，待审核'))
        else:
            return create_response(ret)


@vpc_ns.route('/bindAddress')
