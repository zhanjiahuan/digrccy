...
usdt_ns = Namespace("usdt", description="USDT冲提币接口")
...
class CreateAccount(Resource):
    ...
    def post(self):
        ...
        # digiccy account
        bind_secret, bind_account = usdt_wallet.accountCreate(stellar_address)
...
class Withdraw(Resource):
    ...
    def post(self):
        ...
        if not usdt_wallet.is_syncing():
            return create_response(CodeMsg.CM(1020, 'usdt节点未同步,请勿执行提现操作'))

        # 提币地址校验
        if not usdt_wallet.is_valid_address(digiccy_address):
